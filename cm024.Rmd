...
1. Three possible explanatory variables:
    ...
    1. A third numerical explanatory variable $x_3$: each state's percent population that voted for Donald Trump (`share_vote_trump`) 
...
Group 1: Two numerical predictors with a `+` 
Group 2: One numerical / one categorical with parallel slopes 
Group 3: One numerical / one categorical interaction model 
Group 4?: Two numerical predictors with a `*`
...
1. The numerical outcome variable $y$ `avg_hatecrimes_per_100k_fbi` is on the z-axis (vertical axis)
1. The two numerical explanatory variables form the "floor" axes. In this case
    1. The first numerical explanatory variable $x_1$ `share_vote_trump` is on of the floor axes.
    1. The second numerical explanatory variable $x_2$ `gini_index` is on the other floor axis.
    
    parallel slopes
    
```{r eval = FALSE}
simple_mod <- lm(avg_hatecrimes_per_100k_fbi ~ share_pop_hs,
                 data = hate_demo)
ggplot(hate_demo, aes(x = share_pop_hs, 
