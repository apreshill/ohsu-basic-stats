...
See [Chapter 8 in ModernDive](https://moderndive.com/8-sampling.html#polls).
...
Evaluate the following competing hypotheses:
...
In order to look to see if the observed sample mean of `r cupid_bar %>% pull()` is statistically greater than $\mu_0 = 178.06$, we need to account for the sample size.  We also need to determine a process that replicates how the original sample of size `r nrow(cupid)` was selected.  
...
1. Sample with replacement from our original sample of `r nrow(cupid)` men and repeat this process 1,000 times, 
...
1. Shift the center of this distribution over to the null value of `r mu0`. (This is needed since it will be centered at `r cupid_bar %>% pull()` via the process of bootstrapping.)
1. Use this distribution to observe our $p$-value. Recall this is a right-tailed test so we will be looking for values that are greater than or equal to `r cupid_bar %>% pull()` for our $p$-value.
...
1. Create a confidence interval for the unknown population parameter $\mu$ using our "OKCupid" sample data using *bootstrapping*. 
    - Note that we don't need to shift this distribution since we want the center of our confidence interval to be our point estimate $\bar{x}_{obs} = `r cupid_bar %>% pull()`$.
1. Is `r mu0` is contained in this confidence interval as a plausible value of $\mu$ (the unknown population mean)?
1. Is the entire interval larger than `r mu0`.  
1. Does this match with our hypothesis test results of rejecting the null hypothesis in favor of the alternative ($\mu > 178.06$)?
1. Complete this sentense:

> We are 95% confident the true mean mean height for all US men is between ---- and ---- centimeters.

Which confidence interval is more narrow and hence more precise- your original sample or sample 2a? As a consequence, which would you rather use to estimate your population parameter? Discuss reasons why one interval is wider than the other.
...
> The average height of US males is `r round(x_bar1, 2)`. Does the data in sample 2a provide convincing evidence that average height of US males in this sample is different from `r round(x_bar1, 2)`? Use a significance level of 5%.
...
1. Create a bootrapped distribution of sample means based on sample `2a` (use `set.seed(1701)`).
1. Calculate the 95% confidence interval. 
1. Does it contain `r round(x_bar1, 2)`?
1. How many bootstrap samples produced means as or more extreme than the sample mean I got?
...
(percentile_ci2 <- mh2a_means %>% 
  get_ci())


boot_mean <- mh2a_means %>% 
  summarize(mean_of_means = mean(stat)) %>% 
  pull()
...
null_2a %>% 
  visualize(obs_stat = cupid_bar, direction = "two_sided") 

null_2a %>%
  get_pvalue(obs_stat = cupid_bar, direction = "two_sided")

null_mu <- 178.06
lower_ci <- null_mu - abs(null_mu - boot_mean)
...
null_dist <- cupid %>%
  specify(response = height) %>%
  hypothesize(null = "point", mu = 178) %>% # hypothesize step
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "mean")

null_dist %>% 
  visualize(obs_stat = 182, direction = "greater")
null_dist %>%
  get_pvalue(obs_stat = x_bar1, direction = "two_sided")
```
...
Follow along with [ModernDive Appendix B.5](http://moderndive.netlify.com/b-appendixb).

Bind president sample and cupid sample together with indicator for sample. Then do two-sample t test

```{r include = FALSE}
mh_3a <- read_csv(here::here("data", "president_heights_new.csv")) %>% 
  select(height = `height(cm)`)

mh_3 <- cupid %>% 
  bind_rows(mh_3a, .id = "sample") %>% 
